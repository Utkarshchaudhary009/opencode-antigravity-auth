...
import { runCliOAuthAuthorization, runQuickOAuthAuthorization } from "./plugin/cli-login-flow";
import { getRuntimeAntigravityModels } from "./plugin/model-catalog";
...
import { extractVerificationErrorDetails } from "./plugin/verification";
import type {
  GetAuth,
  LoaderResult,
  PluginClient,
...
function canFallbackAcrossGeminiQuota(reason?: RateLimitReason): boolean {
...
export const createAntigravityPlugin = (providerId: string) => async (
  ...
  // Initialize runtime metadata from the local Antigravity app when available.
  // Falls back to remote version discovery and finally baked-in values.
  ...
  const googleSearchTool = tool({
    ...
    async execute(args, ctx) {
      ...
      let routeState: ProjectContextResult["routeState"] | undefined;
      try {
        const searchContext = await ensureProjectContext({
          ...auth,
          access: accessToken,
          expires: auth.expires ?? Date.now() + 60_000,
        });
        projectId = searchContext.effectiveProjectId;
        routeState = searchContext.routeState;
      } catch {
      }
  ...
  return {
    ...
    provider: {
      ...
      async models(provider, ctx) {
        return getRuntimeAntigravityModels(ctx.auth, provider);
      },
    },
    auth: {
    provider: providerId,
      ...
      return {
        ...
        async fetch(input, init) {
          ...
          while (true) {
            ...
            const endpointOrder = headerStyle === "gemini-cli"
              ? [ANTIGRAVITY_ENDPOINT_PROD]
              : getCloudCodeEndpointOrder(projectContext.routeState);
            ...
            for (let i = 0; i < endpointOrder.length; i++) {
              ...
              const currentEndpoint = endpointOrder[i];
              ...
              try {
                ...
                if (shouldRetryEndpoint && i < endpointOrder.length - 1) {
            ...
            options: [
              {
                label: "Standard",
                value: "false",
                hint: "Default Antigravity individual flow",
              },
              {
                label: "GCP ToS",
                value: "true",
                hint: "Enterprise or GCP ToS flow",
              },
            ],
          },
        ],
        authorize: async (inputs?: Record<string, string>) => {
          const forceGcpTos = inputs?.gcpTos === "true" || process.env.OPENCODE_ANTIGRAVITY_GCP_TOS === "1";

          if (inputs) {
            const noBrowser = inputs.noBrowser === "true" || inputs["no-browser"] === "true";
            return runCliOAuthAuthorization({
              client,
              providerId,
              forceGcpTos,
              useManualMode: noBrowser || shouldSkipLocalServer(),
              openBrowser,
              log,
              accountManager: activeAccountManager,
            });
          }

          return runQuickOAuthAuthorization({
            client,
            providerId,
            forceGcpTos,
            openBrowser,
